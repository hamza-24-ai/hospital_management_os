...
* Uses a condition variable (`pthread_cond_t`) to efficiently sleep and wake the Scheduler when beds become available.
* [cite_start]Enforces hard limits on ICU and Isolation capacities using counting semaphores (`sem_t`).

### Phase 4: Memory Management
* Simulates the hospital ward as a contiguous array of memory units.
* Allocates beds using the Best-Fit algorithm (with First-Fit and Worst-Fit toggles available via command-line flags).
* Logically merges adjacent free blocks upon discharge (Coalescing).
* Dynamically calculates external fragmentation and simulates internal fragmentation via Paging (2-unit page size).
